...
class Camera(
    ...
    fun lookAt(target: Vector3f) {
        ...
        rotation.y =
                Math.toDegrees(Math.atan2(direction.z.toDouble(), direction.x.toDouble())).toFloat()

        val horizontalLength =
                Math.sqrt((direction.x * direction.x + direction.z * direction.z).toDouble())
                        .toFloat()
        rotation.x =
                Math.toDegrees(Math.atan2(-direction.y.toDouble(), horizontalLength.toDouble()))
                        .toFloat()
